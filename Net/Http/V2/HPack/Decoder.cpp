...
const uint8_t * Decoder::DecodeHeaderblock(const uint8_t * pos, const uint8_t * end, std::vector<std::pair<std::string, std::string>>& headerlist)
{
	while (pos != end)
	{
		if ((*pos & 0x80) != 0)
...
const uint8_t * Decoder::DecodeInteger(const uint8_t * pos, uint64_t & integer, uint8_t bits)
{
...
const uint8_t * Decoder::DecodeHuffmanString(const uint8_t * pos, std::string & string, long long length)
{
...
const uint8_t * Decoder::DecodeString(const uint8_t * pos, std::string & string)
{
	uint64_t length;
